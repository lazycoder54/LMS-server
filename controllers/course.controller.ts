...
export const addReview = catchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (course) {
        course.ratings = avg / course.reviews.length; //(if we two reviews, one user gave 5 and another one gave 4 so 5+4) 9/2 = 4.5 ratings
      }
...
export const addReplyToReview = catchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      await redis.set(courseId, JSON.stringify(course), "EX", 608400); //7days
